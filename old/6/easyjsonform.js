...
class EasyJsonFormFieldDate extends EasyJsonFormField {
    ...
    validate() {
        return (this.mandatory && this.value === "") ? ['validation.error.mandatory'] : [];
    }
}
...
class EasyJsonForm {
    constructor(id, structure = null, style = null, options = null) {
        if (id) this.id = id;
        else throw new Error('Обязательное поле');
    ...
    builderGet() {
        if (!this.builder) {
            ...
            for (const [type, classs] of Object.entries(EasyJsonForm.registeredClasses)) {
                let elemDR = this.element('a', 'BuilderToolbarButton');
    ...
    static dictionary = {
        "builder.message.delete": "Вы уверены, что хотите удалить элемент в позиции {{position}}?",
        "builder.message.label.name.already.in.use": "Этот Поле уже используется. Пожалуйста, выберите другой.",
        "builder.message.label.name.cannot.be.numeric": "Поле не может содержать только цифры.",
        "common.export.compound.field": "{{1st-level-label}} - {{2nd-lebel-label}}",
        "common.helptext": " <small>({{help-text}})</small>",
        "common.helptext.min.length.by.character": "min. символов: {{min}}",
        "common.helptext.max.length.by.character": "max. символов: {{max}}",
        "common.helptext.min.length.by.word": "min. words: {{min}}",
        "common.helptext.max.length.by.word": "max. words: {{max}}",
        "common.helptext.mandatory": "обязательный",
        "common.helptext.separator": ", ",
        "common.label.new": "Новое {{field-type}} {{field-number}}",
        "common.value.no": "Нет",
        "common.value.yes": "Да",
        "item.file": "Файл",
        "item.file.helptext.filetypes": "file types: {{file-types}}",
        "item.file.helptext.filetypes.all": "Все виды файлов",
        "item.file.helptext.maxsize": "Макс. размер файла: {{size}} MB",
        ...
        "item.properties.items.help": "Один на строку",
        "item.properties.mandatory": "Обязательное",
        "item.number": "Числовое поле",
        "item.text": "Текстовое поле",
        "item.date": "Дата",
        "item.text.button.dr": "Добавить элемент",
        "item.text.character.count": "{{chars}} символов",
        "item.text.properties.length.max": "Максимальная длина",
        "item.text.properties.length.measure": "Ограничить длину",
        "item.text.properties.length.measure.bycharacter": "Символов",
        "item.text.properties.length.measure.byword": "Слов",
        "item.text.properties.length.measure.no": "Нет",
        "item.text.properties.length.min": "Минимальная длина",
        "item.text.properties.multiline": "Многострочность (текстовая область)",
        "item.text.word.count": "{{words}} слов введено",
        "validation.error.mandatory": "{{field-label}} обязательное поле",
        "validation.error.please.select.one.option": "Пожалуйста, выберите хотя бы один вариант",
        "validation.error.less.than.minimum.chars.accepted":"Меньше символов, чем минимально допустимое",
        "validation.error.more.than.maximum.chars.accepted":"Больше символов, чем максимально допустимое",
        "validation.error.less.than.minimum.words.accepted":"Меньше слов, чем минимально допустимый",
        "validation.error.more.than.maximum.words.accepted":"Больше слов, чем максимально допустимое",
