      ...
      <div class="row-fluid">
        <div class="span2">
			...
			<div class="alert alert-warning" align="center" style="font-family: tahoma, verdana, arial; padding-bottom:20px; background-color:#cce5ff; color: #1387d4">
		  	<span style="font-family:tahoma;color:red;font-size:13px"></span> Advanced features are available in Licensed Version. <br /><br />
			<strong>
		  		<a style="padding:10px" class="btn-primary" target="_blank" href="https://www.gridphp.com/download/">Get License Today!</a>
		  	</strong>
	        </div>
							...
							<div class="accordion-inner">
								...
								foreach($v as $f) 
								{
									...
									if (filesize("demos/$k/$f") < 10)
									{
										...
										$url = "https://www.gridphp.com/demo/demos/$k/$f";
										$star = " <font color='red'>*</font>";
										$ver .= "jQuery('#div_version').html('Available in licensed version. Currently loaded from www.gridphp.com');";
									...
									if ( strstr(strtolower($fname),"bootstrap") !== false || strstr(strtolower($fname),"html") !== false )
										$target = "_blank";
									else
										$target = "demo_frame";
										
									echo "<a href='$url' onclick=\"$ver;jQuery('#code').load('index.php?file=/$k/$f'); $('#grid-demo-tabs a:first').tab('show');\" target='$target'>$fname</a>$star<br/>";
								}
								?>
							</div>
						</div>				
            ...
            <div class="span12">
				...
				<ul class="nav nav-tabs" id="grid-demo-tabs">
					...
					<li class="pull-right">
						...
						<select id="select_theme" class="input-medium">
							...
							<?php
							$black = array("metro-black","dark-one","black-tie","dark-hive","dot-luv","trontastic","vader","ui-darkness");
				...
				<div class="tab-content" id="grid-demo-tabs-content">
					...
					<div id="demo" class="tab-pane fade in active">
		        		...
		        		<div onclick="window.open('//www.gridphp.com/download/');" id="div_version" style="cursor:pointer; text-align:center; padding:5px; margin:auto; width:60%; font-family: tahoma; padding:5px; background-color: #005ccc; letter-spacing:1px; color: white;"></div>
						</span>
					<iframe style="overflow:auto" onload="iframeLoaded(this)" name="demo_frame" frameborder="0" width="100%" height="800" src="demos/promo/customers.php"></iframe>
					</div>
		...
		<script>
			...
			jQuery('#code').load('index.php?file=/promo/customers.php');
					...
					if(iFrameID.contentDocument){
						if (iFrameID.contentDocument.body)
							if (iFrameID.height != iFrameID.contentDocument.body.scrollHeight)
								iFrameID.height = iFrameID.contentDocument.body.scrollHeight + 175;
					} else {
						iFrameID.height = iFrameID.contentWindow.document.body.scrollHeight + 185 + "px";
					}
				}
